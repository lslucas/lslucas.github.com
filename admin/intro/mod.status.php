<?php
 ...
 $sql_guarda = "SELECT ${var['pre']}_status FROM ".TABLE_PREFIX."_${var['path']}";
 ...
 if ($ok) {
	 ...
	 $novoStatusT = $status==1?'não está mais exibindo na home':'exibindo na home';
	 ...
	 $qry_status->bind_param('s', $res['item']); 
		 ...
		 if ($qry_status->execute()) {
			echo "O <b>Vídeo de Introdução</b> agora está <b>${novoStatusT}</b>";
     }
