<?php
    if(!isset($res['col']) && isset($_GET['col']))
      $res['col'] = $_GET['col'];

    if(!isset($res['item']) && isset($_GET['item']))
      $res['item'] = $_GET['item'];


    $res['ref'] = '_'.$var['pre'].'_id';
    $var['pre'] = 'roi';
    $var['path'] = 'r_obr_imagem';
    

    $var['path_imagem']   = PATH_IMG.'/obra';
    $var['path_original'] = PATH_IMG.'/obra/original';
    $var['path_thumb']    = PATH_IMG.'/obra/thumb';
 ...
 $sql_guarda = "SELECT ${res['pre']}_id id,$sql_field field FROM ".TABLE_PREFIX."_${res['prefix']} WHERE ${res['pre']}_id=?";
